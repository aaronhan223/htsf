    ...
    if skipFitting == False:
        ...
        for node in range(nForecasts):
            ...
            with contextlib.redirect_stdout(open(os.devnull, "w")):
                ...
                if capF is None:
                    growth = 'linear'
                    m = Prophet(growth, changepoints1, n_changepoints1, 0.8, yearly_seasonality, weekly_seasonality, daily_seasonality, holidays, 'additive', seasonality_prior_scale, \
                                holidays_prior_scale, changepoint_prior_scale, mcmc_samples, interval_width, uncertainty_samples)
                else:
                    growth = 'logistic'
                    m = Prophet(growth, changepoints, n_changepoints, 0.8, yearly_seasonality, weekly_seasonality, daily_seasonality, holidays, 'additive', seasonality_prior_scale, \
                                holidays_prior_scale, changepoint_prior_scale, mcmc_samples, interval_width, uncertainty_samples)
